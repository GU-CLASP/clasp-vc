...
import { getConnectionDetails } from "./api.js";
...
function buildParticipantList(room) {
  ...
  for (const [id, original] of originals.entries()) {
    ...
    if (relay) {
      if (id === local.identity) {
        // The publisher should not see their own delayed relay.
        continue;
      }
      list.push({
...
export default function App() {
  ...
  async function onJoin() {
    ...
    try {
      ...
      const details = await getConnectionDetails({ inviteId, key, name });
      setConn(details);
    } catch (e) {
      setStatus("error");
